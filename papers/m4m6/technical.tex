...
may either need to create a new world (add a label part) or we may need to
match existing worlds (labels).
In the first case, we create a label part that is uniquely
...
\oops\ employs an input language for formulas implemented using the SableCC~\citep{gagnon1998} compiler generator for Java. Propositions are input as
...
understood infix notation for logical formulas. Table~\ref{tab:input} shows \oops\ {\sc ascii} equivalents for different logical operators, as well as their precedences; lower numbers indicate stronger binding.
